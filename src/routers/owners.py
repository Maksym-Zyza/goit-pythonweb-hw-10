...
from src.schemas.owners import ResponseOwner, OwnerModel, OwnerModelRegister
from src.repository import owners as repository_owners
from src.repository.auth import get_current_user

router = APIRouter(
    prefix="/owners", tags=["owners"], dependencies=[Depends(get_current_user)]
)


@router.get(
    "/",
    response_model=List[ResponseOwner],
)
...
@router.get("/{owner_id}", response_model=OwnerModelRegister)
...
@router.post(
    "/", response_model=OwnerModelRegister, status_code=status.HTTP_201_CREATED
)
async def create_owner(body: OwnerModelRegister, db: Session = Depends(get_db)):
    owner = await repository_owners.create_owner(body, db)
    return owner


@router.put("/{owner_id}", response_model=OwnerModelRegister)
async def update_owner(
    body: OwnerModelRegister, owner_id: int = Path(ge=1), db: Session = Depends(get_db)
):
